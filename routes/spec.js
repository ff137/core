...
import { ENABLE_PLAYER_CACHE, ES_SEARCH_PERCENT, READONLY_POSTGRES_URL } from "../config";
import buildMatch from "../store/buildMatch";
import buildStatus from "../store/buildStatus";
import { write } from "../store/cacheFunctions";
import db, { first, raw, select } from "../store/db";
import { getDistributions, getHeroBenchmarks, getHeroItemPopularity, getHeroRankings, getItemTimings, getLaneRoles, getMetadata, getMmrEstimate, getPeers, getPlayer, getPlayerHeroRankings, getPlayerMatches, getPlayerRatings, getProPeers, getTeamScenarios, insertMatch } from "../store/queries";
import { getJob } from "../store/queue";
import redis, { get as _get, hgetall, mget, rpush, setex, zrangebyscore, zrevrange } from "../store/redis";
import search from "../store/search";
import searchES from "../store/searchES";
import utility, { isRadiant as _isRadiant, checkIfInExperiment, generateJob, getData, mergeObjects } from "../util/utility";
import { heroObject, leagueObject, matchObject, playerObject, teamObject } from "./objects";
import { accountIdParam as _accountIdParam, againstHeroIdParam, dateParam, excludedAccountIdParam, gameModeParam, havingParam, heroIdParam, heroIdPathParam, includedAccountIdParam, isRadiantParam, laneRoleParam, leagueIdPathParam, lessThanMatchIdParam, limitParam, lobbyTypeParam, mmrAscendingParam, mmrDescendingParam, offsetParam, patchParam, projectParam, regionParam, significantParam, sortParam, teamIdPathParam, winParam, withHeroIdParam } from "./params";
import playerFields from "./playerFields.json";
import { duration, player_slot, radiant_win } from "./properties";
...
const spec = {
  ...
  components: {
    ...
    parameters: {
      // ...[...params1, ...params2, ...params3].reduce((acc, param) => {
  ...
  paths: {
              ...
              "application/json; charset=utf-8": {
                schema: {
                  ...
                  properties: {
                    ...
                    radiant_win: {
                      $ref: "#/components/schemas/RadiantWin",
                    },
                    radiant_xp_adv: {
    ...
    "/players/{account_id}/wl": {
      get: {
        ...
        parameters: [
          {
            $ref: "#/components/parameters/PlayerParams",
          }
        ],
        responses: {
          200: {
              ...
              "application/json; charset=utf-8": {
                schema: {
                  ...
                  properties: {
                    ...
                    league_name: matchObject.properties.league_name, //todo
                    cluster: {
                      description: "cluster",
                      type: "integer",
                    },
        ...
        parameters: [
          {
            name: "match_id",
            in: "query",
            description: "Match IDs (array)", // array?
            required: true,
            schema: {
              type: "integer",
            },
            ...
            content: {
              "application/json; charset=utf-8": {
                schema: {
                  oneOf: [
                    {
                      nullable: true //still to be verified if necessary
                    },
                    {
                      type: "object",
                      additionalProperties: {
