...
function genRandomNumber(byteCount, radix) {
    return BigInt('0x' + randomBytes(byteCount).toString('hex')).toString(radix)
}

async function start() {
    // Get the current max_match_id from postgres, subtract 200000000
    let max = (await raw(`select max(match_id) from public_matches`))?.rows?.[0]?.max;
    ...
    while(true) {
        ...
        let result = await execute(`select match_id, player_slot, stuns, token(match_id) from player_matches where token(match_id) >= ? and player_slot = 1 limit 500 ALLOW FILTERING;`, [randomBigint.toString()], {
            prepare: true,
            fetchSize: 500,
            autoPage: true,
          });
        ...
        await Promise.all(ids.map(id => execute(`DELETE from player_matches where match_id = ?`, [id], {
            prepare: true,
        })
        ));
    } catch(e) {
