 ...
 * */
import { each, parallel, series } from "async";
import moment from "moment";
import { PUBLIC_SAMPLE_PERCENT } from "../config";
import db, { raw, select, transaction } from "../store/db";
import queries from "../store/queries";
import { runQueue } from "../store/queue";
import { expireat, hincrby, zadd, zremrangebyrank } from "../store/redis";
import utility, { getAnonymousAccountId as _getAnonymousAccountId } from "../util/utility";
...
function updateRecord(field, match, player) {
  ...
  zremrangebyrank(`records:${field}`, "0", "-101");
  const expire = moment().add(1, "month").startOf("month").format("X");
  expireat(`records:${field}`, expire);
}
...
function updateTurbo(match, cb) {
  for (let i = 0; i < match.players.length; i += 1) {
    ...
    if (heroId) {
      ...
      hincrby("turboPicks", heroId, 1);
      if (win) {
        hincrby("turboWins", heroId, 1);
      }
    }
  }
  expireat("turboPicks", moment().endOf("month").unix());
  expireat("turboWins", moment().endOf("month").unix());
  cb();
}
